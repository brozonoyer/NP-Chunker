...
from syntagrus_morphology import morph
...
class SynTagRusCorpusReader(XMLCorpusReader):
    ...
    def fileids(self, subdir=None):
        ...
        if subdir is None:
            ...
            return [f for f in super(SynTagRusCorpusReader,\
                    self).fileids() if f.startswith(subdir)]
        else:
    ...
    def raw(self, fileids=None):
        ...
        if fileids is None:
            ...
            fileids = [fileids]
        return concat([open(f, encoding = 'windows-1251').read()
                       for f in self.abspaths(fileids)])
    ...
    def parsed_sents(self, fileids=None):
        ...
        def normalize(string):
            """ replace whitespaces/empty string by underscore(s) """ # fantom
            if string:
                return string.replace(' ', '_')
            return '_'
        def normalize_morph(string): # string: feature value
            """ 'value' -> 'category=value' if possible """
            if string in morph:
                return '='.join([morph[string],string])
            return string
