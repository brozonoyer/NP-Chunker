...
def is_dominated(g,j,i): # g: dependency graph, i,j: node addresses
    """ if j is dominated by i (True/False) """
    if 'head' in g.nodes[j]:
        head = g.nodes[j]['head']
        if head!=None:
            if head==i:
                return True
            else:
                return is_dominated(g,head,i)
    return False
...
#https://stackoverflow.com/questions/22490366/cmp-isnt-woking-for-me-python
def cmp(a,b):
    return (a > b) - (a < b)
            ...
            if is_nonprojective_edge(g,h,d):
                ...
                for i in r[1:-1]:
                    if not is_nonprojective_edge(g,h,i):
                        base_index = i - int(k*(cmp(i,h)+1)/2)
